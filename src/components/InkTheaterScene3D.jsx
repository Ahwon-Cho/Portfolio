...
import { createBrushTexture, createInkMaterials, paintInkWash, simplifyInkFigures, finishInkAct } from './inkWashStyle'
import * as THREE from 'three'
import { AHWON_MARK, getMarkPoint, getMarkWritingProgress, MARK_WRITING_STROKES, MARK_WRITING_END } from '../data/ahwonMark'
import {
  createCoffeeScene, createWorkspaceScene,
...
const SIDE_ELEVATION = THREE.MathUtils.degToRad(16)
...
export default function InkTheaterScene3D({ carouselStep = 0, activeIndex = 1, bubbleRef, copyRef, onIntroComplete }) {
  ...
  useEffect(() => {
    ...
    const materials = createInkMaterials()
    const surfaceMaterial = paintInkWash(new THREE.MeshBasicMaterial({ color: 0xf4f1e7, transparent: true }), { texture: brushTexture, strength: 0.16 })
    const edgeMaterial = paintInkWash(new THREE.MeshBasicMaterial({ color: 0xdedfcf, transparent: true }), { texture: brushTexture, strength: 0.22 })
    const markMaterial = new THREE.MeshPhysicalMaterial({ color: 0x516d83, roughness: 1, emissive: 0x708e9c, emissiveIntensity: 0 })
    paintInkWash(markMaterial, { texture: brushTexture, strength: 1 })
    const markBlue = new THREE.Color(0x516d83)
    const inlayColor = new THREE.Color(0xcbd2c3)
    const shadowMaterial = new THREE.ShadowMaterial({ color: 0x505967, opacity: 0.1 })
    ...
    world.add(turntable)
    scene.add(world)
    const floor = new THREE.Mesh(new THREE.CylinderGeometry(STAGE_RADIUS, STAGE_RADIUS, 0.22, 128), surfaceMaterial)
    floor.name = 'circular-stage-floor'
    floor.position.y = -0.11
    floor.castShadow = true
    floor.receiveShadow = true
    const edge = new THREE.Mesh(new THREE.CylinderGeometry(STAGE_RADIUS - 0.06, STAGE_RADIUS - 0.14, 0.1, 128), edgeMaterial)
    ...
    ].map(simplifyInkFigures).map(fitAct).map((anchor) => finishInkAct(anchor, brushTexture))
    ...
    const paintProgress = anchors.map(() => 0)
    let time = 0
    ...
    const updateBubble = () => {
      ...
      const settled = introFinished && turnTime >= TURN_DURATION && anchors[index].visible
        && anchors[index].userData.inkReveal.value >= 0.995
    ...
    const animate = () => {
      ...
      anchors.forEach((anchor, index) => {
        ...
        const opacity = focus * Math.min(1, firstReveal * 5)
        anchor.visible = opacity > 0.001
        // The set remains grounded: brushwork reveals it, not a vertical pop.
        anchor.position.y = 0
        anchor.userData.fadeMaterials.forEach((item) => { item.opacity = item.userData.baseOpacity * opacity })
        if (reduceMotion) paintProgress[index] = 1
        else if (!introFinished) paintProgress[index] = firstReveal
        else if (!selected && !anchor.visible) paintProgress[index] = 0
        else if (selected && focus > 0.98) paintProgress[index] = Math.min(1, paintProgress[index] + delta / 1.2)
        anchor.userData.inkReveal.value = smooth(paintProgress[index], 0, 1)
        const settled = introFinished && (reduceMotion || turnTime >= TURN_DURATION)
          && anchor.userData.inkReveal.value >= 0.995
      ...
      anchors.forEach((anchor) => anchor.userData.inkWorldToAct.value.copy(anchor.matrixWorld).invert())
      updateBubble()
